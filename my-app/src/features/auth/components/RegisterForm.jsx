...
// Đặt ở module scope + memo: gõ vào 1 ô thì chỉ ô đó render lại
...
export default function RegisterForm() {
  ...
  const handleChange = useCallback((e) => {
    ...
    // Xóa lỗi của field đang sửa và lỗi chung. Nếu errors không đổi thì React bỏ qua render.
    setErrors((prev) =>
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      setLoading(true);
      try {
        // confirmPassword chỉ dùng ở client, không gửi lên backend
        ...
        // Backend trả session luôn -> đăng nhập tự động, GuestRoute chuyển về trang home
        setSession(session);
      } catch (error) {
