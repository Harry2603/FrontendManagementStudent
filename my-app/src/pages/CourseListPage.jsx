            ...
            <tbody className="divide-y divide-slate-100">
              {courses.length === 0 ? <tr><td colSpan="4" className="px-5 py-12 text-center text-sm text-slate-500">No courses found.</td></tr> : courses.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50">
                  <td className="px-5 py-4 text-sm font-semibold text-blue-600"><button type="button" onClick={() => navigate(`/courses/${item.id}`)} className="cursor-pointer hover:underline">{item.courseCode}</button></td>
                  <td className="px-5 py-4 text-sm font-medium text-slate-800"><button type="button" onClick={() => navigate(`/courses/${item.id}`)} className="cursor-pointer text-left hover:text-blue-600 hover:underline">{item.courseName}</button></td>
