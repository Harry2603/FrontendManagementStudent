  ...
  return (
    <section className="w-full space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div className="flex items-center gap-3"><button type="button" onClick={() => navigate(-1)} aria-label="Back to courses" className="cursor-pointer rounded-lg p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-800"><ArrowLeft size={19} /></button><div><h1 className="mt-1 text-3xl font-bold text-slate-900">Course Management</h1><p className="mt-1 text-sm font-semibold text-blue-600">{course.courseCode}</p><p className="font-semibold text-slate-900">{course.courseName} <span className="font-normal text-slate-500">({course.credits} credits)</span></p></div></div>
        <button type="button" onClick={openSectionModal} className="inline-flex cursor-pointer items-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-700"><Plus size={17} /> Create course section</button>
      </div>
      ...
      {sectionModalOpen && <Modal title={`Create course section — ${course.courseCode}`} onClose={closeSectionModal}><form onSubmit={submitSection} className="space-y-5 p-6">
        ...
        <div className="flex justify-end gap-3"><button type="button" onClick={closeSectionModal} disabled={sectionSubmitting} className="cursor-pointer rounded-lg px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-100 disabled:cursor-not-allowed">Cancel</button><button type="submit" disabled={sectionSubmitting} className="inline-flex cursor-pointer items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60">{sectionSubmitting && <LoaderCircle size={16} className="animate-spin" />}Create course section</button></div>
      </form></Modal>}
    </section>
  );
}
